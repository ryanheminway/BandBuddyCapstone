    ...
    while (TRUE)
    {
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            if (FD_ISSET(sd, &readfds))
            {
                ...
                {
                    parse_header(buffer, destination, cmd, stage_id, payload_size);
                    if (destination == BACKBONE_SERVER)
                    {
                        switch (cmd)
                        {
                        ...
                        //TODO: Send message through big brother
                        send_through_message(sd, destination, cmd, stage_id, payload_size, client_socket[destination]);
                    }
                }
            }
        }
