...
# (NOTE Ryan Heminway) Replaced old rnn_cell function with my own rnn_cell, below
#   Attempts to remove some of the incompatible TF-Lite operators which are used by the contrib_rnn libraries
...
# (NOTE Ryan Heminway) My own definition for rnn_cell function, using keras LSTMCells which are supposed
#   to be more compatible with TF-Lite
def rnn_cell(rnn_cell_size, dropout_keep_prob, residual, is_training=True):
  #Builds an LSTMBlockCell based on the given parameters.
  dropout_keep_prob = dropout_keep_prob if is_training else 1.0
  cells_keras = []
  for i in range(len(rnn_cell_size)):
    if is_training:
        cell_keras = tensorflow.keras.layers.LSTMCell(rnn_cell_size[i], dropout=dropout_keep_prob)
    else: 
        cell_keras = tensorflow.keras.layers.LSTMCell(rnn_cell_size[i])
    cells_keras.append(cell_keras)
  return tensorflow.keras.layers.StackedRNNCells(cells_keras)
