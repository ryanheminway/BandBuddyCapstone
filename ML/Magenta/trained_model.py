  ...
  def __init__(self, config, batch_size, checkpoint_dir_or_path=None,
    ...
    with tf.Graph().as_default():
      ...
      model.build(
          ...
          is_training=False)
      # Input placeholders
      # (NOTE: Ryan Heminway) I assigned names to all inputs and outputs
      self._temperature = tf.placeholder(tf.float32, shape=(), name="temperature")

      if self._config.hparams.z_size:
        self._z_input = tf.placeholder(
            tf.float32, shape=[batch_size, self._config.hparams.z_size], name="z_input")
      else:
        self._z_input = None
      ...
      self._inputs = tf.placeholder(
          tf.float32,
          shape=[batch_size, None, self._config.data_converter.input_depth], name="input_sequence")
      self._controls = tf.placeholder(
          tf.float32,
          shape=[batch_size, None, self._config.data_converter.control_depth], name="control_sequence")
      self._inputs_length = tf.placeholder(
          tf.int32,
          shape=[batch_size] + list(self._config.data_converter.length_shape), name="sequence_length")
      self._max_length = tf.placeholder(tf.int32, shape=(), name="max_length")
      # Outputs
      self._outputs, self._decoder_results = model.sample(
          ...
          **sample_kwargs)
      self._outputs = tensorflow.identity(self._outputs, name="output_sequence")
      if self._config.hparams.z_size:
      ...
      """if True:
        ...
        for v in tf.global_variables():
            var_name = v.name[:-2]  # Strip ':0' suffix.
            # (NOTE: Ryan Heminway):
            #   After updating LSTM cells, the model had incorrect names for the following tensors
            #   Found correct tensor names using inspect_checkpoints.py script
            var_name = re.sub(r"BasicDecoderStep", "decoder/while/BasicDecoderStep", var_name)
            var_name = re.sub(r"stacked_rnn_cells/", "encoder/cell_0/bidirectional_rnn/fw/fw/while/stacked_rnn_cells/", var_name)
      ...
      # (NOTE Ryan Heminway) Using tf.saved_model.simple_save to save the model in a "saved_model.pb" format
      # Save model
      tf.saved_model.simple_save(self._sess, "./saved_test_model/",
                                 inputs={"input_sequence": self._inputs,
                                         "sequence_length": self._inputs_length},
                                 outputs={"output_sequence": self._outputs})
      """tf.saved_model.simple_save(self._sess, "./saved_test_model/",
                                 inputs={"temperature": self._temperature,
                                         "z_input": self._z_input,
                                         "input_sequence": self._inputs,
                                         "sequence_length": self._inputs_length,
                                         "max_length": self._max_length},
                                 outputs={"output_sequence": self._outputs})"""
